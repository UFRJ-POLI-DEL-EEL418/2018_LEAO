...
public class PrimeiroAcesso extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultadosDTO resultados = null;

        String datahoraref = null;
        SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        Calendar c = Calendar.getInstance();
        Date d = c.getTime();
        datahoraref = df.format(d);
System.out.println("1 =========== "+datahoraref);
        resultados = BaseDeDadosDAO.buscarResultadosPorData(datahoraref);

        request.setAttribute("datahoraref", datahoraref);
